# Sistema de Usuarios - R5 (OAuth Social: Google, Facebook, X, Discord + BBDD SQL 3FN)

Proyecto integral desarrollado bajo el estándar de calidad **`miojaja`** con **React 19**, **Vite**, **TypeScript**, **Autenticación Federada Multi-Proveedor (Google, Facebook, X, Discord)** y backend en **Node.js (ESM)** conectado a una base de datos relacional **SQLite normalizada en Tercera Forma Normal (3FN)**.
...
| :--- | :---: | :--- |
| **Ingreso Social Multi-Proveedor** | ✅ | Autenticación federada con **Google (GIS)**, **Facebook**, **X (Twitter)** y **Discord**. Persistencia de `auth_provider`, `provider_id` y `avatar` en BBDD SQL (3FN). |
...
Desde la raíz del proyecto (`R5`):
```bash
npm start
```
> Ejecuta simultáneamente el backend en el puerto `5000` y el frontend en el puerto `7888` mediante `concurrently`.
...
npm run dev
```
Aplicación disponible en: `http://localhost:7888`
