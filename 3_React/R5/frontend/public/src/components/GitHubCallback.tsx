...
export function GitHubCallback() {
  ...
  const [message, setMessage] = useState<string>('Verificando autorización con GitHub...');
  ...
  useEffect(() => {
    ...
    const processAuth = async () => {
      try {
        ...
        if (errorDesc) {
          setStatus('error');
          setMessage('GitHub rechazó la solicitud de autenticación.');
          setDetails(errorDesc);
          if (window.opener) {
            window.opener.postMessage(
              { type: 'GITHUB_AUTH_ERROR', message: errorDesc },
              '*'
            );
          }
          return;
        ...
        if (code) {
          setMessage('Intercambiando código con GitHub y generando sesión SQL...');
          const redirectUri = `${window.location.origin}${window.location.pathname}`;

          const result = await loginWithGitHub(code, redirectUri);
          ...
          if (result.success && result.user) {
            ...
            setStatus('error');
            setMessage(result.message || 'Error al validar el código con GitHub.');
        ...
        setStatus('error');
        setMessage('No se recibió el código de autorización de GitHub en la URL de retorno.');
        setDetails(`URL actual: ${window.location.href}`);
      } catch (err: any) {
        console.error('Error en GitHubCallback:', err);
        setStatus('error');
        setMessage(err.response?.data?.message || err.message || 'Error inesperado durante la autenticación con GitHub.');
      }
    };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-slate-900/90 backdrop-blur-md p-8 rounded-3xl border border-slate-800 text-center shadow-2xl space-y-4">
        ...
        {/* Ícono de Estado Branded de GitHub */}
        <div className="w-16 h-16 rounded-2xl bg-[#24292e] text-white border border-slate-700 flex items-center justify-center mx-auto shadow-lg shadow-black/50">
        ...
        <div>
          <h2 className="text-lg font-bold text-white tracking-tight">GitHub OAuth</h2>
