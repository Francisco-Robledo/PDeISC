...
import { Lock, Mail, LogIn, AlertCircle, Sparkles, Route, Eye, EyeOff } from 'lucide-react';
...
export function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    setServerError(null);
    // Trimming de correo y contraseña
  ...
  return (
    <div className="max-w-md mx-auto my-8 p-6 bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-800 transition-colors">
      ...
      <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100 dark:border-slate-800">
        ...
        <span className="text-[11px] text-slate-400 font-mono">/router/login</span>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <button
          ...
          className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 active:scale-98 text-white font-semibold rounded-xl text-sm shadow-md transition disabled:opacity-50"
        >
      ...
      <SocialAuthButtons
        onSuccessRedirect={() => navigate('/router/dashboard')}
      ...
      <div className="mt-5 text-center text-xs text-slate-500 dark:text-slate-400">
        ...
        <Link to="/router/register" className="text-indigo-600 dark:text-indigo-400 font-semibold hover:underline">
          Regístrate aquí
        </Link>
      </div>
