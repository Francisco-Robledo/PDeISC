...
export const EditTaskPage: React.FC = () => {
  ...
  const { getTaskById, updateTask, showToast } = useTasks();

  const task = id ? getTaskById(id) : undefined;

  usePageTitle(task ? `Editar: ${task.title}` : 'Editar Tarea');
      ...
      <div className="py-16 text-center space-y-6 animate-in fade-in duration-300">
        ...
        <div className="space-y-2 max-w-md mx-auto">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            No es posible editar una tarea que no existe o fue eliminada.
          </p>
        </div>
        <Link
          to="/"
    ...
    <div className="py-6 sm:py-8 space-y-6 max-w-3xl mx-auto animate-in fade-in duration-300">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-5 sm:p-7 md:p-8 shadow-sm space-y-6 transition-colors">
        ...
        <TaskForm
          ...
          onCancel={() => navigate(`/task/${id}`)}
          submitColorClass="bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 shadow-indigo-600/25"
        />
      </div>
    </div>
  );
