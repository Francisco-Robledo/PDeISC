...
import { Route, ArrowLeft, CheckCircle2, AlertCircle } from 'lucide-react';
...
export function UsersPage() {
  ...
  // Reemplazo estricto de window.confirm() por ConfirmModal integrado en la UI
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            <Link
              to="/router/dashboard"
              className="text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition flex items-center space-x-1 text-xs"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              <span>Volver a Dashboard</span>
            </Link>
            <span className="text-slate-300 dark:text-slate-700">&middot;</span>
            <span className="inline-flex items-center space-x-1 text-xs font-semibold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/60 px-2 py-0.5 rounded-full border border-indigo-200 dark:border-indigo-800">
              <Route className="w-3 h-3" />
              <span>/router/users</span>
            </span>
          </div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
            CRUD completo conectado a la base de datos relacional SQLite mediante peticiones Axios protegidas con JWT.
          </p>
        </div>
      </div>

      {/* Alerta de notificación en UI (sin alert()) */}
      {feedback && (
        <div
