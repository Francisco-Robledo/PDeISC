...
// --- ELIMINAR ELEMENTO DEL ORIGINAL ---
...
// Render Original con índices y botón de borrar
...
// Render de la Copia (Solo lectura)
const renderCopia = (containerId, arrayData, colorClass) => {
    const container = document.getElementById(containerId);
    container.innerHTML = ''; 
...
const actualizarUI = (data) => {
    renderArrayOriginal('arrayNumeros', data.numeros, 'primary', 'numeros');
    renderArrayOriginal('arrayPeliculas', data.peliculas, 'danger', 'peliculas');
    renderArrayOriginal('arrayItems', data.items, 'success', 'items');
};
...
// Formularios para agregar al original
...
configurarFormAdd('formPeliculas', 'inputPeliculas', '/api/peliculas');
configurarFormAdd('formItems', 'inputItems', '/api/items');

// --- BOTONES PARA EJECUTAR SLICE (GET) ---
const configurarSlice = (btnId, endpoint, boxResultadoId, containerResultadoId, colorClass) => {
    document.getElementById(btnId).addEventListener('click', async () => {
        try {
            const res = await fetch(endpoint);
            const data = await res.json();
            if (res.ok) {
                // Mostramos la caja de resultado
                document.getElementById(boxResultadoId).classList.remove('d-none');
                // Pintamos la copia
                renderCopia(containerResultadoId, data.copia, colorClass);
...
configurarSlice('btnSliceNumeros', '/api/numeros/slice', 'boxResultadoNumeros', 'resNumeros', 'primary');
configurarSlice('btnSlicePeliculas', '/api/peliculas/slice', 'boxResultadoPeliculas', 'resPeliculas', 'danger');
configurarSlice('btnSliceItems', '/api/items/slice', 'boxResultadoItems', 'resItems', 'success');