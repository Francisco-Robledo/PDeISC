...
const renderArray = (containerId, arrayData, colorClass, tipo) => {
    ...
    arrayData.forEach((item, index) => {
        ...
        if (tipo === 'personas') {
            ...
            badge.innerHTML = `<span class="text-start">{ nombre: '${item.nombre}', edad: <strong class="text-${colorClass}">${item.edad}</strong> }</span>`;
        } else {
...
// --- EJECUTAR SORT (GET) ---
const configurarSort = (btnId, endpoint) => {
    document.getElementById(btnId).addEventListener('click', async () => {
        try {
            const res = await fetch(endpoint);
            const data = await res.json();
            if (res.ok) {
                // Al volver a renderizar con el estado global actualizado, 
                // se ve físicamente cómo las etiquetas cambian de lugar.
                actualizarUI(data.estadoGlobal);
                showNotification(data.mensaje, true);
            } else {
                showNotification(data.error, false);
            }
        } catch (error) { showNotification('Error al ordenar', false); }
    });
};

configurarSort('btnSortNumeros', '/api/numeros/sort');
configurarSort('btnSortPalabras', '/api/palabras/sort');
configurarSort('btnSortPersonas', '/api/personas/sort');